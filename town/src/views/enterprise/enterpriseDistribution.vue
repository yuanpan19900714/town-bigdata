<template>
    <div style="height: 100%;width: 100%;">
        <div style="height: 100%;width: 100%;" id="enterpriseDistribution"></div>
    </div>
</template>

<script>
    import {
        initEnterpriseDistribution
    } from '@/assets/js/enterprise/enterpriseDistribution';
    ...
    export default {
        ...
        methods: {
            init: function (year) {
                const that = this;
                new Promise(resolve => {
                    this.$axios.get("http://localhost:8080/data/enterpriseDistribution", {
                        params: {
                            year: year
                        }
                    }).then(res => {
                        if (res.status === 200) {
                            let data = res.data;
                            if (data.year === year && data.type === "enterpriseDistribution") {
                    ...
                    myChart1 = echarts.init(document.getElementById("enterpriseDistribution"));
            ...
            initData: function (targetData, myChart1) {
                ...
                const option1 = initEnterpriseDistribution(currMonthData, monthStr);//行业分布
