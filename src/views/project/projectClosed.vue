<template>
    <div style="height: 100%;width: 100%;">
        <div id="closedDetail" style="float: left;height: 100%;width: 100%;"></div>
    </div>
</template>
...
<script>
    import {
        initClosedDetail,
    } from '@/assets/js/project/projectClosed';
    ...
    export default {
        ...
        methods: {
            init: function (year) {
                const that = this;
                new Promise(resolve => {
                    this.$axios.get("http://localhost:8080/data/projectClosed", {
                        params: {
                            year: year
                        }
                    }).then(res => {
                        if (res.status === 200) {
                            let data = res.data;
                            if (data.year === year && data.type === "projectClosed") {
                    ...
                    myChart1 = echarts.init(document.getElementById("closedDetail"));
            ...
            initData: function (targetData, myChart1) {
                ...
                const option1 = initClosedDetail(currMonthData, monthStr); //已结项目
